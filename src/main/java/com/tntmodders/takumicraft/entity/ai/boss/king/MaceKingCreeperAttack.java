...
public class MaceKingCreeperAttack extends AbstractKingCreeperAttack {
    ...
    public void clientTick(LivingEntityRenderState state, TCKingCreeper creeper, int swell, PoseStack pose, MultiBufferSource bufferSource, float renderTick) {
        ...
        pose.scale(f, f, f);
        Minecraft.getInstance().getItemRenderer().renderStatic(creeper, mace, ItemDisplayContext.NONE, false, pose, bufferSource, creeper.level(), 0xffffff, LivingEntityRenderer.getOverlayCoords(state, 0.0F), creeper.getId());
        pose.popPose();
    }
