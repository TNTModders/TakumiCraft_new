...
public class TCCreeperIlluminationBlock extends AbstractTCAntiExplosionBlock implements ITCRecipe {
    ...
    public void addRecipes(TCRecipeProvider provider, ItemLike itemLike, RecipeOutput consumer) {
        provider.saveRecipe(itemLike, consumer, ShapedRecipeBuilder.shaped(RecipeCategory.BUILDING_BLOCKS, TCBlockCore.CREEPER_ILLUMINATION, 8).define('#', TCBlockCore.CREEPER_BOMB).define('B', Blocks.BEACON).pattern("BBB").pattern("B#B").pattern("BBB").unlockedBy("has_creeperbomb", TCRecipeProvider.hasItem(TCBlockCore.CREEPER_BOMB)));
    }
}
